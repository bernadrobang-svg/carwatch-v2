...
def recommend(conn, account, req, root: str = ROOT, csrf: str = "", flash_key: str = "-",
    ...
    ★★ 탭 1 — ★ **여섯 축의 합**으로 세운다 (예산 95 · 주행 107 · 연식 80 ·
      ★ 색(외) 25 · 색(내) 10 · 크기 31).  ★ 09-04 — ★ 규격이 여섯이다
...
def admin_collect(conn, account, req, root: str = ROOT, csrf: str = "",
    ...
    if req.get("method") == "POST":
        ...
        # ★★★★★ 09-02 명령서 16 (`S46-243` · RULES.md 2) — ★ **판을 돌려야 나온다.**
        #   ★ 마스터께서 09-01 에 수집을 누르셨는데 ★ 매물만 **21,156건**으로 늘고
        ...
        return redirect(
            ...
            f"{got.opened} 를 열었습니다 (origin=browser){queued}{_todo}", flash_key,
            csrf=csrf)
