...
def state_counts(conn: sqlite3.Connection) -> dict:
    ...
    sqls = {
        ...
        # ★★★★★★ 09-04 — ★ **마스터가 받으신 것만 센다**.
        #   ★ 마스터 — 「★ 내가 **매일 브라우저로 수집했는데**
        #     ★ ★ 왜 **5일째 수집 안 됐다**고 하지?」
        #   ★ 실측 09-04 — ★ 엔카 목록 원문이 ★ `origin='browser'` **389건**이고
        #     ★ ★ 마지막이 ★ **09-04 15:54** 다.  ★ 마스터께서 어제 받으셨다.
        #     ★ ★ ★ `collector`(서버)는 ★ **08-31** 이 마지막이다 — ★ 그 뒤로 407 이다.
        #   ★★ 그런데도 ★ 「5일째 멈췄다」가 떴다 — ★ **잣대가 틀렸다**.
        #     ★ ★ 아래 `encar_407_at` 이 ★ **서버가 두드려 받은 407** 을 보는데,
        #       ★ 서버 판은 ★ **지금도 돈다** — ★ 그래서 ★ `at407` 이 늘 최신이고
        #       ★ ★ 마스터가 아무리 받으셔도 ★ **다시 「막혔다」**가 된다.
        #   ★★★ 그러니 ★ `list_at` 은 ★ **`origin='browser'` 만** 센다 —
        #     ★ 「마스터가 며칠째 못 받으셨나」가 ★ 사람이 겪는 것이다
