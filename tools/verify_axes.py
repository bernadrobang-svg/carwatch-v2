...
def main() -> int:
    ...
    checks = [
        ...
        ("value.market", "market_median%", "1-1", hand_market, "desc",
         "시세 대비 (중앙값−가격)/중앙값"),
        ("value.origin", "origin_price", "1-2", hand_depreciation,
        ...
        ("state.my_cost", "record_my_cost", "2-4", hand_repair, "asc",
         "자차 수리비 원"),
        ...
        ("state.integrity", "integrity_%", "2-8", hand_integrity, "point",
         "계기판·구조변경·튜닝"),
        ("history.use", "%", "3-1", hand_usage, "label", "용도 (셋 대조)"),
        ("history.not_join", "not_join_%", "3-2", hand_not_join, "asc",
         "자차 미가입 비율"),
        ("history.owner", "owner_%", "3-3", hand_owner, "asc",
         "소유자 변경 횟수"),
        ("history.seizing", "detail_seizing", "3-4", hand_lien, "label",
         "압류·저당"),
        ("taste.trim", "trim_origin_price", "4-1", hand_trim, "ratio",
         "트림 순위 ÷ 전체"),
        ("taste.option", "options_choice_price", "4-2", hand_options,
         "ratio", "옵션가 ÷ 그 차종 P90"),
        ...
        ("taste.fitting", "%", "6", hand_picked, "point", "지정 옵션"),
