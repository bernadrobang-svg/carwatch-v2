...
#   ★★★★★ 09-03 — ★ **8 을 쟀다.  ★ 답은 「올리지 마라」다** (`--bursts 8` 실측)
#        묶음 1 → 받음 10 · 봇차단 3회 **0**
#        묶음 2 → 받음 10 · 봇차단 3회 **0**
#        묶음 3 → 받음 10 · 봇차단 3회 **0**
...
#      ★ ★ ★ 그러므로 ★ `DEFAULT_BURSTS` 는 ★ **4 가 맞다.  ★ 올리지 않는다.**
#      ★ 08-30 의 「4묶음 40건」과 ★ 오늘의 37건이 ★ 같은 자리를 가리킨다.
DEFAULT_BURSTS = 4
...
# ★★★★★ 09-03 (2부 S1) — ★ 4 → **12**.
#   ★ 실측 09-03 (`G80_25T`) — ★ 매물은 ★ **46쪽**에서 끝나는데
#   ★ ★ 「진짜 끝」 신호(3,585B)는 ★ **56쪽**에서야 온다.
#   ★ ★ ★ 그 사이 ★ **빈 쪽이 아홉**이다 (69~71KB 인데 `carSeq` 가 0).
#   ★★ 4 로는 ★ 50쪽에서 포기해 ★ `done=False` 가 된다 —
#     ★ ★ 그러면 ★ 훑기(`sweep`)가 ★ **그 차종을 통째로 건너뛴다**.
#     ★ ★ ★ 그것이 ★ 「끝까지 받은 묶음 **34/35**」의 정체였다 [실측 09-03].
#   ★★★ 「빈 쪽이 몇 장이면 포기」를 ★ **늘리기만** 한다 —
#     ★ ★ 「빈 쪽이 나오면 끝난 것으로 친다」로 바꾸지 **않는다**.
#     ★ ★ ★ 사이트가 잠깐 빈 쪽을 주면 ★ **산 차를 죽인다** (규격이 막은 자리다)
TAIL_LIMIT = 12
