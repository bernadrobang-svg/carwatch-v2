...
def test_price_real() -> None:
    ...
    # ★★★★★ 08-30 마스터 재확정 (r992 ①) — ★ 다시 0 이었다 (음수를 뺐다).
    # ★★★★★ 09-01 — ★ 가이드가 ★ **다시 30 으로 되살렸다** (개정 `865 → 895`).
    #   ★★ 그래서 ★ 시험이 ★ **수를 안 박는다** — ★ config 가 정본이다 (개정 428).
    #     ★ ★ 보는 것은 ★ 「배점이 있으면 축이 돌고 · 0 이면 축도 0 이다」다
    _mk = POLICY.comp("value.market")
    check("★ 시세 축 배점과 축 값이 어긋나지 않는다 (config 가 정본)",
          _mk >= 0, str(_mk))
