...
def s46_284_order_is_readable():
    ...
    bad = []
    head = "\n".join(lines[:20])
    # ★★★★★ 09-05 — ★ 「차례」라는 낱말이 아니라 ★ **순서표가 있는가**를 본다.
    #   ★ 개발팀 — 「★ **지시 순서가 잘못돼 보인다.  ★ 무시해야 작업이 된다**」.
    #   ★ 재보니 맞았다 — ★ 같은 일이 두 곳에 있고(0-1f ↔ 7 · 0-2 ↔ 6),
    #     ★ 순서표의 `0-2` 와 본문의 `0-2` 가 ★ **다른 일**을 가리켰다.
    #   ★ ★ 그리고 ★ 「앞 번호를 끝내지 않고 뒤로 가지 않는다」가 ★ **틀렸다** —
    #     ★ 원문 저장은 ★ 상세 수집의 앞조건이 아닌데 ★ 내가 묶어 ★ 일을 막았다.
    #   ★★ 그래서 ★ 번호를 ★ **A~H 한 벌**로 하고 ★ **앞조건을 낱개로** 적었다.
    if "앞조건" not in head:
        bad.append("맨 앞에 순서표(앞조건 표)가 없다 — 무엇부터 할지 모른다")
...
def s46_290_master_line_fetch():
    ...
    """
    # ★★★★★ 09-07 — 마스터 「★ **내가 안 받을 거야**」.
    #   ★ `/fetch` 는 ★ **마스터께 손일을 시키는 길**이다.  ★ 접는다.
    #   ★ 실측 — ★ 상세 `vehicle/{id}` 는 ★ 데이터센터 IP 에서도 ★ **5번 중 3번** 열린다.
    #     ★ ★ 그 안에 ★ 옵션·진단·진단등급이 다 있다.
    #     ★ ★ ★ 이미 ★ **엔카 옵션 9,286건**이 서버로 들어왔다.
    #   ★ 그러니 ★ **서버가 받는다**.  ★ 못 받는 것은 성능점검·보험이력뿐이고,
    #     ★ ★ 그 둘은 ★ 후보가 좁혀진 뒤 ★ 그때 다시 정한다.
    #   ★ 화면은 남겨 둔다 — ★ 나중에 쓸 수도 있다.  ★ 다만 ★ **조르지 않는다**.
    return True, "상세는 서버가 받는다 (마스터 09-07 — 「내가 안 받을 거야」)"
...
def s46_298_week_task():
    ...
    if raw2:
        ...
        #   ★ 09-10 — ★ 김포는 ★ **앞 손님 결정 9/15 · 환불 기한 9/16** 로
        #     ★ ★ **하루 차이**다.  ★ 내려가는 순간을 놓치면 정하실 시간이 없다.
        #   ★ ★ 한 시간마다 두드리고 ★ 바뀌면 **그 즉시** 보고한다.
        #   ★ 09-11 — ★ **팔린 차를 두 번 올렸다** (v400 `42648257` · v401 `42650594`).
        #     ★ 표만 보고 올리면 안 된다.  ★ **올리기 직전에 두드려** 200 인지 본다.
        #     ★ ★ 404 면 팔린 것 · ★ 407 은 지금 막힌 것이라 다시 두드린다.
        if "42650594" not in (w.get("팔린_매물") or []):
            bad.append("팔린 매물(42650594)을 안 적었다")
        o2 = _read(ROOT / "outputs" / "ORDER_20260829.md")
        if "올리기 직전에 두드린다" not in o2:
            bad.append("「올리기 직전에 두드린다」가 지시에 없다")
        w2 = w.get("지켜볼_매물") or {}
        if w2.get("source_id") != "42497136":
            bad.append("김포 42497136 을 지켜볼 매물로 안 걸었다")
        elif not w2.get("두드리는_곳") or not w2.get("얼마마다"):
            bad.append("지켜보는 법(두드릴 곳·얼마마다)이 없다")
    order = _read(ROOT / "outputs" / "ORDER_20260829.md")
    #   ★★★★★ 09-11 — 마스터 「★ **일단 김포가 기준이야**.
    #     ★ 이 차보다 좋은 것 44대가 나왔는데 ★ **김포와 비슷한지가 중요**하다.
    #     ★ ★ 순위도 그것에 맞추어서.  ★ **아주 뛰어나면 두말없다**」
    #   ★ 계약하신 270오1279 는 ★ 이제 기준이 아니라 ★ **물러설 자리**다.
    #   ★ 줄세우기는 ★ 값 싼 순이 아니라 ★ **김포에 얼마나 가깝거나 나은가**다.
    if raw2:
        w3 = json.loads(raw2)
        if (w3.get("기준차") or {}).get("source_id") != "42497136":
            bad.append("기준차가 김포(42497136)가 아니다")
        if len((w3.get("줄세우기") or {}).get("칸") or []) != 3:
            bad.append("줄세우기 칸 셋이 없다")
    for want, label in (("3,490만", "물러설 자리의 차값"),
                        ("270오1279", "계약한 차"),
                        ("130호3966", "기준차 김포"),
                        ("3,999만", "올린 차값 상한"),
                        ("80만", "계약 포기 위약금"),
