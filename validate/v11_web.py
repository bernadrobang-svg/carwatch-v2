...
def _menu_shape_checks(conn, rid):
    ...
    # ★★★★★ 09-01 가이드 확정 (개정 1051 · `03_이력.md:1259`) —
    #   ★ 「★ 메뉴 다섯 vs 여섯 → ★ **여섯**  (09-01 에 추천이 들어갔다 ·
    #     ★ `V11-157` 은 08-27 것) …  ★ **규격이 바뀌면 검사도 바꾼다**」
    #   ★ `UI_REVIEW` 의 「메뉴 다섯」을 ★ 가이드가 ★ 여섯으로 고쳤다.
    #   ★★ 메뉴는 ★ 여전히 ★ **마스터 몫**이다 — ★ 개발측이 스스로 늘리지 않는다
    limit = 6                      # 규격 「여섯을 넘는가」
    acc = Account(1, ROLE_ADMIN, "마스터")
    got = menu_items(acc)
    bad157 = []
    ...
    for width, body in _media_blocks(css):
        ...
        for rule in re.finditer(r"([^{}]+)\{([^{}]*)\}", body):
            ...
            if "thead" in sel or "#peek" in sel:
                ...
                continue
            # ★★ 08-28 — ★ 메뉴는 ★ **표의 값 칸이 아니다** (명령서 82장).
            #   ★ 시안대로 ★ 좁은 폭에서 메뉴를 아래에 고정하면서
            #     ★ ★ 곁말(`em`)과 「잠김」 딱지를 접었다 — ★ 탭은 이름만 낸다.
            #   ★ 이 검사가 지키는 것은 ★ 「값이 사라지지 않는가」다 —
            #     ★ ★ 메뉴 이름표는 그대로 있고 ★ 꾸밈만 접는다
            if ".nav" in sel:
                continue
