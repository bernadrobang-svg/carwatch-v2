...
def halt_if(rep: StepReport, raw_rows: int) -> StepReport:
    ...
    # ★★★★★ STEP 49c (개정 1122 · 가이드 확정 09-03) —
    #   ★ **`⑥ not_requested` 만 남은 실패는 ★ 「진행 중」이다.**
    #   ★★★ 개발측 v363 물음 — 「★ 규격이 ★ 한 판에 다 못 받으면 `failed` 라 하는데
    #     ★ ★ 엔카 **136,431번**을 ★ 한 판에 받을 길이 없다.
    #     ★ ★ ★ 그러면 ★ `S46-241` 이 ★ **늘 붉다**」.
    #   ★★ 가이드 답 — 「★ **늘 붉은 검사는 ★ 아무도 안 본다.  ★ 그것이 더 나쁘다**」.
    #     ★ ⑥ 은 ★ **이어 받으면 준다** (`STEP 49a`) — ★ 판을 안 죽인다.
    #     ★ ★ ⑤·④·③ 은 ★ **그대로 실패**다 — ★ 이어 받아도 ★ 안 준다.
    #   ★ 「아직 안 끝났다」는 ★ 회차의 「끝까지 받은 묶음 N/M」이 말한다 — ★ 검사가 아니다
    #   ★★★★★ **물렸다 (09-04) — ★ 시험이 막았다.  ★ 그대로 적는다.**
    #     ★ 나는 ★ 「⑥ 만 남으면 ★ 안 죽인다」로 고쳤다가 ★ **되돌렸다**.
    #     ★ ★ 까닭 — ★ `tests/test_pipeline.py` 의 ★ **「v1 사고 (208 중 76) → 중단」**이
    #       ★ ★ ★ 바로 ★ **⑥ 만 걸리는 자리**다 (① 208 = 76 + 132 로 맞고 · ② 도 맞다).
    #     ★ ★ ★ ★ 곧 ★ ⑥ 을 풀면 ★ **이 프로젝트가 막으려던 그 사고**가 통과한다.
    #   ★★ 규격(STEP 49c)과 ★ 불변식이 ★ **부딪힌다** — ★ 회차에 여쭐 것으로 냈다.
    #     ★ 내가 보는 가름은 ★ 「이어 받을 수 있는 판인가」인데
    #     ★ ★ `StepReport` 에 ★ 그것을 말하는 칸이 없다 — ★ 지어내지 않는다 (규칙 2)
