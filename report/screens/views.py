...
class ListingFilter:
    ...
    #   ★ **「짝지어진 것만」**.  ★ 누르면 ★ 여러 사이트에 함께 올라온 차만 본다.
    #   ★★ 순위는 ★ **안 건드린다** — ★ 348점 차례 그대로다.  ★ 거르기만 한다.
    #     ★ ★ 「짝이 있다」는 ★ 좋은 차라는 뜻이 아니다 (평균 38점 낮다) —
    #     ★ ★ ★ 정렬에 섞으면 ★ **나쁜 차가 먼저 온다** (규격이 금지한다)
    paired: bool = False
...
class RecommendRow:
    ...
    rank: int | None = None
    # ★★★★★ 09-04 (1부 1-4 를 추천에도 · `RULES.md` 2) — ★ 사진이 없으면 ★ **까닭**.
    #   ★★★ 마스터 09-04 — 「★ 브라우저로 ★ **그림이 없거나** 가격이 없거나 링크가 없거나」
    #   ★★ 실측 09-04 (브라우저) — ★ `/recommend` 60카드 중 ★ **23장**이 빈 자리인데
    #     ★ ★ **까닭이 한 줄도 없었다** — ★ `/listings` 는 내고 있다.
    #     ★ ★ ★ 「조용히 빼지 않는다」가 ★ 추천에만 안 걸려 있었다
    photo_note: str | None = None
