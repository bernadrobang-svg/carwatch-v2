...
| R | 층 | 요구사항 | 출처 | 규격 | 소스 | 화면 | 검사 | 상태 |
|---|---|---|---|---|---|---|---|:--:|
| ST-002 | `[저장]` | 자연키는 UNIQUE 로 남긴다 | `[기술]` 중복 방지는 그대로 작동한다 | `STEP 30` | 미구현 | — | 검사 없음 | ✗ |
| ST-003 | `[저장]` | VIN 은 17자리 · 영숫자 · I·O·Q 미포함 | `[원문]` 국제 규격 | `11-store` VIN | 미확인 | — | 검사 없음 | ◐ |
...
| ST-006 | `[저장]` | `CHECK` 에 넷을 다 넣는다 | `[기술]` V11-40·43 이 이 값을 요구한다 | `STEP 33` | 미확인 | — | 검사 없음 | ◐ |

## 이행 · 트랜잭션

| R | 층 | 요구사항 | 출처 | 규격 | 소스 | 화면 | 검사 | 상태 |
|---|---|---|---|---|---|---|---|:--:|
| ST-007 | `[저장]` | 재작성 전후 행 수를 세고 같은지 확인 | `[기술]` 조용히 잃는 것을 막는다 | `STEP 32b` | 미구현 | — | 검사 없음 | ✗ |
...
| ST-014 | `[저장]` | 2가 실패하면 3을 하지 않는다 | `[기술]` PII 만 남는 고아 행을 만들지 않는다 | `11-store` PII | 미구현 | — | 검사 없음 | ✗ |
| ST-015 | `[저장]` | PII 는 지정 함수로만 읽는다 | `[기술]` `core_pii` 직접 SELECT 금지 | `11-store` PII | 미구현 | — | 검사 없음 | ✗ |
...
| ST-020 | `[판정]` | NULL 이면 판정 대상이 아니다 | `[마스터]` 개정 271 | `11-store` | 미확인 | 해당 없음 | V2-31 | ○ |
| ST-021 | `[저장·화면]` | 화면에서 갈라 낸다 — 「범위 안 N · 차종 미정 N」 | `[마스터]` 개정 271 — 섞어서 「매물 N건」으로 내지 않는다 | `11-store` | 미확인 | `/notready` | V2-31 · V2-32 | ○ |
...
| R | 층 | 요구사항 | 출처 | 규격 | 소스 | 화면 | 검사 | 상태 |
|---|---|---|---|---|---|---|---|:--:|
| ST-023 | `[판정]` | `NOT_RATED` 이면 반드시 채운다. 등급만 내지 않는다 | `[마스터]` 왜 못 매겼는지가 있어야 한다 | `STEP 37` | 미확인 | `/why` | V5-12 | ○ |
| ST-024 | `[저장·화면]` | 충돌은 「같은 근거 두 값」으로 낸다 | `[마스터]` 개정 — 버리지 않는다 | `11-store` 충돌 | 미확인 | `/why` · L1 | 검사 없음 | ◐ |
| ST-027 | `[저장·화면]` | 확보율은 「원문 기준」과 「컬럼 기준」을 나눠 낸다 | `[판단]` 둘이 다르다. ★ 어느 쪽을 볼지는 마스터 판단 | `11-store` | 미확인 | 해당 없음 | V2-01 | ? |
| ST-026 | `[사전]` | 미확정이 해소되면 본문 서술도 함께 고친다 | `[마스터]` S28 — 표만 고치면 이격이 난다 | `11-store` | 미확인 | 해당 없음 | S28-3 | ○ |

## 마스터께 여쭐 것

| R | 층 | 요구사항 | 출처 | 규격 | 소스 | 화면 | 검사 | 상태 |
|---|---|---|---|---|---|---|---|:--:|
| ST-027 | `[저장]` | 「확보율을 원문 기준과 컬럼 기준으로 나눈다」 | ★ 둘 중 어느 쪽을 화면에 낼지 정해 주십시오 |  |  |  |  | ? |
| ST-028 | `[저장]` | 「`display_name` 이 비면 `login_name`」 | ★ 조사 없음. 다른 방식이 나을 수 있습니다 |  |  |  |  | ? |
