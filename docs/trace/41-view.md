...
| R | 층 | 요구사항 | 출처 | 규격 | 소스 | 화면 | 검사 | 상태 |
|---|---|---|---|---|---|---|---|:--:|
| VW-015 | `[화면]` | 표본 부족 딜러는 점수를 흐리게 | `[마스터]` 점을 0 으로 찍지 않는다 | `STEP 99` | ~ `web/templates/dealers.html` | `/dealers` | V3-26 | ◐ |
...
| R | 층 | 요구사항 | 출처 | 규격 | 소스 | 화면 | 검사 | 상태 |
|---|---|---|---|---|---|---|---|:--:|
| VW-020 | `[저장]` | 축 칩은 한 번에 (`IN`) | `[기술]` 08-16 실측 — 쿼리가 200번 넘었다 | `41-view` | 미구현 | `/listings` | V11-34 | ✗ |
...
| R | 층 | 요구사항 | 출처 | 규격 | 소스 | 화면 | 검사 | 상태 |
|---|---|---|---|---|---|---|---|:--:|
| VW-026 | `[화면]` | 「표시가+부대비용 ≤ 1,500만 이면 전액 현금」 ★ 1,500만은 **총액 상한** | `[마스터]` 354·400 |  | ~ `report/finance.py::build_finance` |  |  | ◐ |
| VW-027 | `[화면]` | ~~「부족액을 표시한다」~~ ★ 폐기 — 개정 400.  「전액 현금」인가 아닌가 둘뿐 | `[마스터]` 400 |  | ~ `report/finance.py::build_finance` |  |  | ◐ |
| VW-028 | `[수집]` | 「`page_size` 를 URL 로 **받는다**」 · 상한 `page_size_max` | `[마스터]` 356 |  | 미구현 |  |  | ✗ |
| VW-029 | `[화면]` | 월 납입에 「추정」 표시를 **붙인다** | `[마스터]` 363 |  | ~ `report/finance.py::price_for_monthly` |  |  | ◐ |
