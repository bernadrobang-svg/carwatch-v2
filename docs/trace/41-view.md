...
| R | 층 | 요구사항 | 출처 | 규격 | 소스 | 화면 | 검사 | 상태 |
|---|---|---|---|---|---|---|---|:--:|
| VW-010 | `[화면]` | 세 값을 한 셀에 — 바로 비교되게 | `[마스터]` 개정 283 — 「시세차가 아니라 시세고 신차가도」 | `41-view` 금융 | 미구현 | 목록 | V11-81 | ○ |
...
| VW-014 | `[판정]` | 시세차는 기대가 대비 % + 절대 금액 | `[마스터]` 개정 283 | `41-view` | 미구현 | 목록 | 검사 없음 | ◐ |

## 경고 · 신뢰

| R | 층 | 요구사항 | 출처 | 규격 | 소스 | 화면 | 검사 | 상태 |
|---|---|---|---|---|---|---|---|:--:|
| VW-015 | `[판정]` | 표본 부족 딜러는 점수를 흐리게 | `[마스터]` 점을 0 으로 찍지 않는다 | `STEP 99` | 미구현 | `/dealers` | V3-26 | ○ |
...
| R | 층 | 요구사항 | 출처 | 규격 | 소스 | 화면 | 검사 | 상태 |
|---|---|---|---|---|---|---|---|:--:|
| VW-020 | `[판정]` | 축 칩은 한 번에 (`IN`) | `[기술]` 08-16 실측 — 쿼리가 200번 넘었다 | `41-view` | 미구현 | `/listings` | V11-34 | ○ |
