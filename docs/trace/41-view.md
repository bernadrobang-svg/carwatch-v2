...
| R | 층 | 요구사항 | 출처 | 규격 | 소스 | 화면 | 검사 | 상태 |
|---|---|---|---|---|---|---|---|:--:|
| VW-026 | `[화면]` | 「표시가+부대비용 ≤ 1,500만 이면 전액 현금」 ★ 1,500만은 **총액 상한** | `[마스터]` 354·400 |  |  |  |  | ◐ |
| VW-027 | `[화면]` | ~~「부족액을 표시한다」~~ ★ 폐기 — 개정 400.  「전액 현금」인가 아닌가 둘뿐 | `[마스터]` 400 |  |  |  |  | ◐ |
| VW-028 | `[수집]` | 「`page_size` 를 URL 로 **받는다**」 · 상한 `page_size_max` | `[마스터]` 356 |  |  |  |  | ◐ |
| VW-029 | `[화면]` | 월 납입에 「추정」 표시를 **붙인다** | `[마스터]` 363 |  |  |  |  | ◐ |
