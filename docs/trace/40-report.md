...
| R | 층 | 요구사항 | 출처 | 규격 | 소스 | 화면 | 검사 | 상태 |
|---|---|---|---|---|---|---|---|:--:|
| RP-005 | `[판정]` | 세율·이율은 config | `[마스터]` 0장 STEP 6 | `40-report` | ✓ `config/finance.json::tax_acquisition_rate` | — | 검사 없음 | ◐ |
...
| R | 층 | 요구사항 | 출처 | 규격 | 소스 | 화면 | 검사 | 상태 |
|---|---|---|---|---|---|---|---|:--:|
| RP-015 | `[화면]` | 전 요소를 `ReportMeta` 에서 가져온다 — 유지 | `[판단]` 403 | `40-report` | ~ `report/views.py::ReportMeta` | 화면 없음 | V8-02 | ◐ |
...
| RP-014 | `[판정]` | csv 헤더에 배점을 표기 — 성분에서 읽는다 (`value.market(100)`) | `[판단]` 412 | `40-report` | ✓ `report/exports/export.py::listing_csv` | — | S28-1 | ○ |

## 마스터께 여쭐 것

| R | 층 | 요구사항 | 출처 | 규격 | 소스 | 화면 | 검사 | 상태 |
|---|---|---|---|---|---|---|---|:--:|
| RP-015 | `[화면]` | 전 요소를 `ReportMeta` 에서 가져온다 — 유지 | `[판단]` 403 |  | ~ `report/views.py::ReportMeta` |  |  | ◐ |
| RP-016 | `[화면]` | 월 납입에 「추정」 표시를 **붙인다** | `[마스터]` 363 |  | ~ `report/finance.py::price_for_monthly` |  |  | ◐ |
...
| RP-021 | `[판정]` | 안 주면 계산하고 「추정」 | `[판단]` 이전등록비는 법정 요율. ★ 요율 조사 필요 | `40-report` | ✓ `report/finance.py::purchase_cost` | — | V11-120 · V11-121 | ○ |
| RP-022 | `[판정]` | 추가 혜택을 따로 — 「10년 보증 포함」 | `[마스터]` **제네시스 인증중고차** | `40-report` | ✓ `report/views.py::PurchaseCostView` | — | V11-120 · V11-121 | ○ |
...
| RP-029 | `[저장]` | 자주 쓰는 쿼리를 **저장한다** | `[마스터]` 358 | `40-report` | ~ `store/dictionary.py:2` | — | 검사 없음 | ◐ |
