...
| R | 층 | 요구사항 | 출처 | 규격 | 소스 | 화면 | 검사 | 상태 |
|---|---|---|---|---|---|---|---|:--:|
| RP-003 | `[저장]` | 「조치」는 사람이 할 수 있는 행동으로 | `[마스터]` 위와 같음 | `40-report` L0 | 미확인 | `/notready` | 검사 없음 | ◐ |
| RP-004 | `[저장·화면]` | 중단 리포트 — 진행분을 낸다 | `[마스터]` 처음부터 다시 도는 게 아님을 알 수 있게 | `40-report` L0 | 미구현 | 화면 없음 | 검사 없음 | ✗ |

## 금융 계산

| R | 층 | 요구사항 | 출처 | 규격 | 소스 | 화면 | 검사 | 상태 |
|---|---|---|---|---|---|---|---|:--:|
| RP-005 | `[판정]` | 세율·이율은 config | `[마스터]` 0장 STEP 6 | `40-report` | 미확인 | — | 검사 없음 | ◐ |
| RP-016 | `[판정·화면]` | 추정 항목은 「추정」으로 표시 | `[판단]` 실비가 아니다. ★ 법적 검토 없음 | `40-report` | 미확인 | 목록 | 검사 없음 | ◐ |
...
| RP-012 | `[화면]` | 임시 파일에 쓰고 `os.replace` | `[기술]` 쓰다 죽으면 반쪽 파일이 남는다 | `40-report` | 미확인 | 화면 없음 | 검사 없음 | ◐ |
...
| RP-014 | `[판정]` | csv 헤더에 배점을 표기 | `[판단]` ★ **규격이 낡았다 — 「price(200)」은 개정 329 로 250 이다** | `40-report` | 미확인 | — | S28-1 | **!** |
...
| RP-019 | `[수집]` | 여러 사이트에 같은 차면 나란히 | `[마스터]` **표시가가 싼 쪽이 실제로 싼 쪽이 아닐 수 있다** | `40-report` | 미확인 | — | V11-121 | ◐ |
| RP-020 | `[수집]` | 사이트가 총액을 주면 그것을 쓴다 | `[원문]` K카는 화면에 낸다 | `40-report` | 미확인 | — | V11-121 | ◐ |
| RP-021 | `[판정]` | 안 주면 계산하고 「추정」 | `[판단]` 이전등록비는 법정 요율. ★ 요율 조사 필요 | `40-report` | 미확인 | — | V11-120 · V11-121 | ◐ |
| RP-022 | `[판정]` | 추가 혜택을 따로 — 「10년 보증 포함」 | `[마스터]` **제네시스 인증중고차** | `40-report` | 미확인 | — | V11-120 · V11-121 | ◐ |
...
| RP-029 | `[저장]` | 자주 쓰는 쿼리를 저장 | `[판단]` 매번 붙여넣게 하지 않는다. ★ 마스터 확인 필요 | `40-report` | 미확인 | — | 검사 없음 | ? |
