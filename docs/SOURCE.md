...
| 파일 | 줄 | 무엇 |
|---|--:|---|
| `validate/v0_guide.py` | 5,126 | 가이드 문서 자체를 검사한다 (V0 계열). |
| `validate/v11_web.py` | 5,049 | V11 표현 계층 검증 (14장 STEP 153). |
| `report/screens/build.py` | 4,025 | 화면 데이터 생성. |
| `web/views.py` | 2,765 | 화면 어댑터 (14장 STEP 142 · 152). |
| `validate/v3_logic.py` | 2,316 | V3 로직 검증 — 판정이 작동하는가 · 변별력이 있는가. |
| `store/core.py` | 1,887 | CORE 저장소 (L4).  사이트 무관 공통 스키마. |
| `collect/runner.py` | 1,722 | 수집 실행 규칙. |
...
| `tools/browser_diff.py` | 159 | ★★★★★ 09-01 마스터 지시 — ★ **브라우저로 시안과 화면을 대조한다.** |
...
### `collect/runner.py` — 1,722줄

```
CollectGroup:67  load_targets:91  collect_groups:104  facet_axes:172  aspect_names:193  check_facet_axes:197  interpret_failure:211  collect_check:240  FailStreak:310  _sleep:344  _log_request:350  _save_issues:361  make_executors:372  classify_in_group:989  _query_key:1018  _group_of:1026  _fuel_of:1041  _badge_of:1047  _pages_for:1053  _dicts:1067  _option_medians:1103  _market_medians:1149  _trim_ladders:1178  _option_base:1195  _site_grade_rules:1225  _listing_config:1241  _listing_values:1266  _option_money:1285  _owned_months:1304  _option_of:1316  _market_of:1324  _group_sums:1334  _origin_lend_table:1362  _origin_keys:1392  _origin_lent:1411  make_score_executors:1431  make_validate_executor:1651  make_registry_executor:1693
```
