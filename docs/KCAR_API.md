...
`SPEC-2026.08.30-r1011` · 2026-08-22 · **가이드가 직접 실측했다 (원칙 4)**
...
# ★★★★★ 08-29 — ★ **보험이력이 상세 안에 다 있다** (가이드가 직접 쟀다)

```
★★★ 마스터 — 「★ **야 그건 네가 챙겨야지.  ★ 그걸 개발에 넘긴 거야?
   ★ 그게 효과적이라 생각했니?  ★ 개발애들은 브라우저로 못 들어간다면서**」
★ ★ **옳다.  ★ 내가 잴 수 있는 것을 넘겼다** — ★ 그래서 직접 쟀다
```

## ★ `data.carhistory` — ★ **칸이 100개다** [실측 08-29]
...
| 사고 자세히 | — | `acdtDcdN11~N23` · `acdtHapnDttm` | 부위·날짜 |

```
★★★ ★ **보험이력 넷(79점)이 ★ 상세 JSON 안에 그대로 있다**
   ★ ★ ★ **따로 부를 창구가 필요 없다** — ★ 우리가 이미 받는 응답 안에 있었다
   ★ ★ ★ 곧 ★ **「안 부른 것」도 아니고 ★ 「안 뽑은 것」이다**
★ ★ `history.not_join`(18점 · 자차 미가입)만 ★ 아직 못 찾았다 — ★ 칸 100개를 더 봐야 한다
★ ★ **성능점검 칸은 없다** — `insp`·`pfmnc`·`diag` 로 걸리는 칸이 ★ 0개다
   ★ ★ 그것은 ★ 점검 경로(`/bc/car-insp/photo/cm`)가 ★ **따로 있다**(개정 887 · 사진뿐)
```

★ **파서가 할 일** — `parse/kcar/mapping.py` 가 ★ `data.carhistory` 를 ★ **안 읽고 있다**.
  ★ ★ 읽기만 하면 ★ **79점이 채워진다**.  ★ K카 천장 71.6% → **80% 대**
