...
★★ ★ 그러므로 ★ 모바일 주소로도 ★ 서버에서는 목록을 못 받는다
   ★ ★ 규격 0장의 「목록을 못 부른다」는 ★ 여전히 맞다
```

```
필수  ★ 개발측이 ★ 브라우저 개발자도구 ★ 네트워크 칸을 열어
      ★ 그 XHR 주소와 ★ 요청 꼴을 ★ 적어라 (★ enc 인지 아닌지 함께)
금지  ★ `enc` 를 푸는 것 · ★ 우회를 만드는 것 (그대로다)
★ ★ XHR 도 enc 면 ★ 지금처럼 ★ 마스터가 `carCd` 를 주시는 길뿐이다
★ ★ enc 가 아니면 ★ 목록을 서버에서 받을 수 있다 — ★ 그러면 K카가 열린다
```
