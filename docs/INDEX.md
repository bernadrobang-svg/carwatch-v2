# INDEX — 지시서 색인

**`python3.11 tools/build_index.py` 가 만든다. 손으로 고치지 않는다.**

```
읽는 법   1  이 표에서 파일을 고른다
         2  소스를 고칠 때는 MAPPING.md 로 챕터를 찾는다
         3  검사를 고칠 때는 CHECKS.md · DB 는 SCHEMA.md
금지     docs/ 전체를 읽는 것
```

**89파일 · 27,873줄 · 1,220KB**

★ 800줄을 넘는 파일 6개 — 폴더로 쪼갭니다 (개정 342 · S28-9)

- guide/01_요구사항.md 841줄
- chapters/00-standard.md 2,172줄
- chapters/41-view.md 933줄
- chapters/61-web.md 2,255줄
- chapters/30-score/f-table.md 1,327줄
- chapters/60-admin/c-tools.md 1,076줄

| 파일 | 줄 | KB |
|---|--:|--:|
| `ENCAR_API.md` | 183 | 7 |
| `KCAR_API.md` | 210 | 6 |
| `README.md` | 57 | 1 |
| `chapters/00-standard.md`  ★ | 2,172 | 93 |
| `chapters/01-arch.md` | 227 | 8 |
| `chapters/10-collect/00-intro.md` | 368 | 14 |
| `chapters/10-collect/a-endpoint.md` | 301 | 11 |
| `chapters/10-collect/b-parse.md` | 96 | 3 |
| `chapters/10-collect/c-mapping.md` | 48 | 2 |
| `chapters/10-collect/d-policy.md` | 218 | 8 |
| `chapters/10-collect/d-record.md` | 587 | 22 |
| `chapters/10-collect/e-catalog.md` | 200 | 7 |
| `chapters/11-store/00-intro.md` | 95 | 3 |
| `chapters/11-store/a-key.md` | 531 | 22 |
| `chapters/11-store/b-core.md` | 708 | 25 |
| `chapters/11-store/c-result.md` | 217 | 7 |
| `chapters/12-dict.md` | 592 | 23 |
| `chapters/13-pipeline.md` | 677 | 26 |
| `chapters/20-verify/00-intro.md` | 194 | 8 |
| `chapters/20-verify/a-frame.md` | 19 | 0 |
| `chapters/20-verify/b-v1v2.md` | 202 | 9 |
| `chapters/20-verify/c-v3v4.md` | 325 | 14 |
| `chapters/20-verify/d-v5.md` | 250 | 8 |
| `chapters/30-score/00-intro.md` | 59 | 1 |
| `chapters/30-score/a-frame.md` | 757 | 29 |
| `chapters/30-score/b-price.md` | 255 | 8 |
| `chapters/30-score/c-spec.md` | 226 | 9 |
| `chapters/30-score/d-history.md` | 365 | 12 |
| `chapters/30-score/e-etc.md` | 225 | 7 |
| `chapters/30-score/f-table.md`  ★ | 1,327 | 50 |
| `chapters/30-score/g-absolute.md` | 142 | 5 |
| `chapters/30-score/h-verdict.md` | 193 | 7 |
| `chapters/31-registry.md` | 673 | 28 |
| `chapters/40-report.md` | 757 | 26 |
| `chapters/41-view.md`  ★ | 933 | 34 |
| `chapters/42-watch.md` | 714 | 26 |
| `chapters/50-multisite.md` | 445 | 15 |
| `chapters/60-admin/00-intro.md` | 219 | 8 |
| `chapters/60-admin/a-auth.md` | 388 | 14 |
| `chapters/60-admin/b-ops.md` | 304 | 11 |
| `chapters/60-admin/c-tools.md`  ★ | 1,076 | 43 |
| `chapters/61-web.md`  ★ | 2,255 | 85 |
| `chapters/61-web/a-common.md` | 61 | 2 |
| `chapters/61-web/b-list.md` | 84 | 3 |
| `chapters/61-web/c-recommend.md` | 42 | 1 |
| `chapters/61-web/d-detail.md` | 293 | 11 |
| `chapters/61-web/e-compare.md` | 52 | 1 |
| `chapters/61-web/f-width.md` | 76 | 2 |
| `chapters/61-web/g-chart.md` | 101 | 4 |
| `chapters/61-web/h-admin.md` | 277 | 9 |
| `chapters/61-web/i-admin-mock.md` | 385 | 17 |
| `chapters/61-web/j-admin-mock2.md` | 495 | 22 |
| `guide/00_개요.md` | 138 | 6 |
| `guide/00_버전.md` | 96 | 4 |
| `guide/01_시작.md` | 280 | 10 |
| `guide/01_요구사항.md`  ★ | 841 | 19 |
| `guide/02_결함대장.md` | 270 | 6 |
| `guide/02_미확정.md` | 76 | 4 |
| `guide/03_원칙지적.md` | 153 | 3 |
| `guide/03_이력.md` | 455 | 120 |
| `guide/04_구성.md` | 37 | 2 |
| `guide/04_질의.md` | 256 | 5 |
| `guide/05_가이드역할.md` | 363 | 12 |
| `guide/05_노션.md` | 188 | 5 |
| `guide/06_오판대장.md` | 164 | 10 |
| `guide/07_밀린일대장.md` | 99 | 4 |
| `ref/00-intro.md` | 25 | 0 |
| `ref/A-check.md` | 68 | 3 |
| `ref/B-config.md` | 405 | 12 |
| `ref/C-fixture.md` | 65 | 2 |
| `ref/D-update.md` | 32 | 1 |
| `ref/E-attach.md` | 526 | 19 |
| `ref/F-scoring.md` | 12 | 0 |
| `ref/G-screens.md` | 12 | 0 |
| `trace/00-standard.md` | 276 | 29 |
| `trace/02-collect.md` | 73 | 7 |
| `trace/05-score.md` | 112 | 11 |
| `trace/11-store.md` | 83 | 6 |
| `trace/12-dict.md` | 61 | 5 |
| `trace/13-pipeline.md` | 61 | 5 |
| `trace/14-web.md` | 124 | 14 |
| `trace/20-verify.md` | 48 | 4 |
| `trace/31-registry.md` | 56 | 3 |
| `trace/40-report.md` | 75 | 7 |
| `trace/41-view.md` | 90 | 7 |
| `trace/42-watch.md` | 85 | 7 |
| `trace/50-multisite.md` | 69 | 7 |
| `trace/60-admin.md` | 201 | 24 |
| `trace/RULES.md` | 312 | 21 |

---

# ★★★ 원천은 ★ 다섯이다 — ★ 나머지는 가리키기만 한다 (개정 664 · 마스터 지시 08-24)

```
★★ 마스터가 ★ 키워드를 뽑아 ★ 흩어진 정도를 재셨다 —
   축 배점 ★ 374회 · 51개 문서   보증 ★ 246회 · 47개   분모 ★ 225회 · 42개
   검사 ★ 2,180회 · 85개         ★ 똑같은 문장이 ★ 3개 이상 문서에 ★ 14개
★ ★ 같은 사실이 ★ 여섯 곳에 손으로 적혀 ★ 서로 어긋났다
```

| 무엇 | ★ 원천 | 나머지는 |
|---|---|---|
| ★ **배점 · 갈래 · 컷** | ★ `config/scoring.json` → `f-table.md`(생성물) | ★ **가리키기만** |
| ★ **사이트** | ★ `config/sites.json` ＋ `config/endpoints.json` | 〃 |
| ★ **차종** | ★ `config/targets.json` | 〃 |
| ★ **검사** | ★ `validate/*.py` → `CHECKS.md`(생성물) | 〃 |
| ★ **칼럼(그릇)** | ★ `sql/ddl/*.sql` → `SCHEMA.md`(생성물) | 〃 |

```
필수  ★ 원천이 아닌 문서는 ★ **사실만 적고 ★ 원천을 가리킨다**
      ★ 예 — 「엔카는 소모품을 안 준다」(사실) ○ · 「그러니 ② 로 채운다」(판정) ✘
금지  ★ DDL 컬럼을 ★ 문서에 손으로 옮겨 적는 것
      ★ 실측 08-24 — `target_key` ★ **29개 문서** · `listing_id` ★ **30개** · `first_seen` ★ **7개**
      ★ ★ `SCHEMA.md` 를 가리켜라
검산  ★ `S46-33` — ★ 같은 문장이 ★ 3개 이상 문서에 있으면 ★ 알린다 (지금 ★ 14개)
```

## ★ 생성물 넷 — ★ 손으로 고치지 않는다

```
`INDEX.md` · `CHECKS.md` · `SCHEMA.md` · `SOURCE.md`  ← ★ `tools/build_index.py` 가 만든다
`f-table.md`                                          ← ★ `tools/gen_table.py` 가 만든다
★★ ★ 08-24 실측 — ★ `build_index.py` 가 ★ **194행에서 죽어 있다**
   ★ ★ 그동안 ★ 넷을 ★ 손으로 관리했다 → ★ 줄 수가 ★ 29곳 어긋났다 (`61-web.md` +2,191)
필수  ★ 생성물을 고치기 전에 ★ **생성기를 돌려 본다**
검산  ★ `S46-32` — ★ 생성물이 낡았으면 ★ 실패
```

---

# ★★ 사이트 규격과 판정 규격 — ★ 08-24 에 INDEX 에 올렸다

```
★★ 이 열일곱이 ★ INDEX 밖에 있었다 (마스터 지적 08-24)
★ ★ 살아 있는 문서인데 ★ 어느 목록에도 없었다 — ★ 새로 오는 사람이 못 찾는다
```

## 사이트 규격 열둘

| 문서 | 무엇 |
|---|---|
| `ENCAR_API.md` | 엔카 — ★ 생성물(`tools/probe_encar.py`).  손으로 쓰지 않는다 |
| `ENCAR_ROBOTS.md` | 엔카 robots — ★ 마스터 판단 대기 |
| `KIA_CPO_API.md` | 기아 인증중고차 |
| `HYUNDAI_CERTIFIED_API.md` | 현대·제네시스 인증중고차 |
| `KBCHACHACHA_API.md` | KB차차차 |
| `KCAR_API.md` | K카 |
| `HEYDEALER_API.md` | 헤이딜러 — ★ 목록 API 500 |
| `BOBAEDREAM_API.md` | 보배드림 |
| `REBORNCAR_API.md` | 리본카 — ★ 모바일 UA 필수 |
| `LEXUS_CERTIFIED_API.md` | 렉서스 인증 |
| `VOLVO_SELEKT_API.md` | 볼보 셀렉트 |
| `BMW_BPS_API.md` | BMW 프리미엄 셀렉션 |

## 가로지르는 것 다섯

| 문서 | 무엇 |
|---|---|
| ★ `MULTISITE_MAPPING.md` | ★ **원문 칸 → `core_listing` 칼럼.  ★ 매핑의 정본** |
| ★ `DEDUP_CROSS_SITE.md` | ★ 같은 차가 여러 사이트에 뜰 때 |
| `TARGET_KEY_MAP.md` | 차종 이름 → 차종 키 |
| ★ `SERVER_SURVIVAL.md` | ★ 서버가 살아 있는가 · ★ 503 가르는 법 |
| ★ `UI_REVIEW.md` | ★ 화면 — 마스터가 짚으신 것 |
| `SOURCE.md` | 원문 갈래 |
| `guide/08_인수인계.md` | 다음 가이드에게 |

```
필수  ★ 문서를 새로 만들면 ★ **그 커밋에서 ★ INDEX 에 올린다**
검산  ★ `S46-30` — ★ `docs/**/*.md` 중 ★ INDEX 가 안 가리키는 것이 있으면 ★ 알린다
      ★ 일부러 하나를 빼서 ★ 잡히는지 보고 ★ 되돌려라
```
