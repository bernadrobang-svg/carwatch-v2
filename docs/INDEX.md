# INDEX — 지시서 색인

```
version  SPEC-2026.08.24-r688
follows  `docs/INDEX.md`
sources  실측 08-24
checks   S46-38 · S46-39
```


**`python3.11 tools/build_index.py` 가 만든다. 손으로 고치지 않는다.**

```
읽는 법   1  이 표에서 파일을 고른다
         2  소스를 고칠 때는 MAPPING.md 로 챕터를 찾는다
         3  검사를 고칠 때는 CHECKS.md · DB 는 SCHEMA.md
금지     docs/ 전체를 읽는 것
```

**110파일 · 35,819줄 · 2,054KB**

★ 800줄을 넘는 파일 6개 — 폴더로 쪼갭니다 (개정 342 · S28-9)

- guide/01_요구사항.md 950줄
- chapters/00-standard.md 2,207줄
- chapters/41-view.md 932줄
- chapters/61-web.md 2,253줄
- chapters/30-score/f-table.md 1,598줄
- chapters/60-admin/c-tools.md 1,076줄

| 파일 | 줄 | KB |
|---|--:|--:|
| `BMW_BPS_API.md` | 225 | 10 |
| `BOBAEDREAM_API.md` | 259 | 11 |
| `CHECKS.md` | 543 | 84 |
| `DEDUP_CROSS_SITE.md` | 139 | 6 |
| `ENCAR_API.md` | 198 | 7 |
| `ENCAR_ROBOTS.md` | 178 | 8 |
| `HEYDEALER_API.md` | 268 | 13 |
| `HYUNDAI_CERTIFIED_API.md` | 389 | 18 |
| `KBCHACHACHA_API.md` | 302 | 15 |
| `KCAR_API.md` | 334 | 16 |
| `KIA_CPO_API.md` | 177 | 9 |
| `LEXUS_CERTIFIED_API.md` | 248 | 12 |
| `MAPPING.md` | 96 | 3 |
| `MULTISITE_MAPPING.md` | 799 | 45 |
| `README.md` | 57 | 1 |
| `REBORNCAR_API.md` | 273 | 14 |
| `SCHEMA.md` | 112 | 5 |
| `SERVER_SURVIVAL.md` | 250 | 12 |
| `SOURCE.md` | 610 | 43 |
| `TARGET_KEY_MAP.md` | 111 | 4 |
| `UI_REVIEW.md` | 417 | 22 |
| `VOLVO_SELEKT_API.md` | 237 | 10 |
| `chapters/00-standard.md`  ★ | 2,207 | 94 |
| `chapters/01-arch.md` | 227 | 8 |
| `chapters/10-collect/00-intro.md` | 368 | 14 |
| `chapters/10-collect/a-endpoint.md` | 301 | 11 |
| `chapters/10-collect/b-parse.md` | 96 | 3 |
| `chapters/10-collect/c-mapping.md` | 48 | 2 |
| `chapters/10-collect/d-policy.md` | 218 | 8 |
| `chapters/10-collect/d-record.md` | 587 | 22 |
| `chapters/10-collect/e-catalog.md` | 200 | 7 |
| `chapters/11-store/00-intro.md` | 95 | 3 |
| `chapters/11-store/a-key.md` | 531 | 22 |
| `chapters/11-store/b-core.md` | 708 | 25 |
| `chapters/11-store/c-result.md` | 217 | 7 |
| `chapters/12-dict.md` | 592 | 23 |
| `chapters/13-pipeline.md` | 677 | 26 |
| `chapters/20-verify/00-intro.md` | 194 | 8 |
| `chapters/20-verify/a-frame.md` | 19 | 0 |
| `chapters/20-verify/b-v1v2.md` | 202 | 9 |
| `chapters/20-verify/c-v3v4.md` | 324 | 14 |
| `chapters/20-verify/d-v5.md` | 250 | 8 |
| `chapters/30-score/00-intro.md` | 59 | 1 |
| `chapters/30-score/a-frame.md` | 758 | 29 |
| `chapters/30-score/b-price.md` | 255 | 8 |
| `chapters/30-score/c-spec.md` | 132 | 4 |
| `chapters/30-score/d-history.md` | 365 | 12 |
| `chapters/30-score/e-etc.md` | 225 | 7 |
| `chapters/30-score/f-table.md`  ★ | 1,598 | 67 |
| `chapters/30-score/g-absolute.md` | 142 | 5 |
| `chapters/30-score/h-verdict.md` | 188 | 7 |
| `chapters/31-registry.md` | 673 | 28 |
| `chapters/40-report.md` | 756 | 26 |
| `chapters/41-view.md`  ★ | 932 | 34 |
| `chapters/42-watch.md` | 714 | 26 |
| `chapters/50-multisite.md` | 445 | 15 |
| `chapters/60-admin/00-intro.md` | 219 | 8 |
| `chapters/60-admin/a-auth.md` | 388 | 14 |
| `chapters/60-admin/b-ops.md` | 304 | 11 |
| `chapters/60-admin/c-tools.md`  ★ | 1,076 | 43 |
| `chapters/61-web.md`  ★ | 2,253 | 85 |
| `chapters/61-web/a-common.md` | 61 | 2 |
| `chapters/61-web/b-list.md` | 84 | 3 |
| `chapters/61-web/c-recommend.md` | 42 | 1 |
| `chapters/61-web/d-detail.md` | 293 | 11 |
| `chapters/61-web/e-compare.md` | 52 | 1 |
| `chapters/61-web/f-width.md` | 76 | 2 |
| `chapters/61-web/g-chart.md` | 101 | 4 |
| `chapters/61-web/h-admin.md` | 277 | 9 |
| `chapters/61-web/i-admin-mock.md` | 385 | 17 |
| `chapters/61-web/j-admin-mock2.md` | 494 | 22 |
| `guide/00_개요.md` | 141 | 6 |
| `guide/00_버전.md` | 169 | 11 |
| `guide/01_시작.md` | 283 | 10 |
| `guide/01_요구사항.md`  ★ | 950 | 26 |
| `guide/02_결함대장.md` | 273 | 6 |
| `guide/02_미확정.md` | 111 | 6 |
| `guide/03_원칙지적.md` | 156 | 3 |
| `guide/03_이력.md` | 685 | 426 |
| `guide/04_구성.md` | 40 | 2 |
| `guide/04_질의.md` | 513 | 19 |
| `guide/05_가이드역할.md` | 767 | 34 |
| `guide/05_노션.md` | 191 | 6 |
| `guide/06_오판대장.md` | 436 | 82 |
| `guide/07_밀린일대장.md` | 260 | 25 |
| `guide/08_인수인계.md` | 290 | 14 |
| `ref/00-intro.md` | 25 | 0 |
| `ref/A-check.md` | 68 | 3 |
| `ref/B-config.md` | 405 | 12 |
| `ref/C-fixture.md` | 65 | 2 |
| `ref/D-update.md` | 32 | 1 |
| `ref/E-attach.md` | 526 | 19 |
| `ref/F-scoring.md` | 12 | 0 |
| `ref/G-screens.md` | 12 | 0 |
| `trace/00-standard.md` | 276 | 29 |
| `trace/02-collect.md` | 73 | 7 |
| `trace/05-score.md` | 112 | 10 |
| `trace/11-store.md` | 83 | 6 |
| `trace/12-dict.md` | 61 | 5 |
| `trace/13-pipeline.md` | 61 | 5 |
| `trace/14-web.md` | 124 | 14 |
| `trace/20-verify.md` | 48 | 4 |
| `trace/31-registry.md` | 56 | 3 |
| `trace/40-report.md` | 75 | 7 |
| `trace/41-view.md` | 90 | 7 |
| `trace/42-watch.md` | 85 | 7 |
| `trace/50-multisite.md` | 69 | 7 |
| `trace/60-admin.md` | 201 | 24 |
| `trace/INDEX.md` | 53 | 1 |
| `trace/RULES.md` | 312 | 21 |

---

# ★★★ `ref/v1-ui/` — ★ **지우지 않는다** (마스터 확정 08-24 · 개정 682)

```
★★ 마스터 — 「★ **UI 학습 시 참고하라고 넣은 거잖아.**  ★ 이제 필요 없나?
   ★ 너 ★ v2 도 막 날렸는데」
★ ★ 가이드가 ★ 「아무 문서도 안 가리킨다 → 고아다 → 지운다」로 ★ **판단했다.  ★ 틀렸다**
   ★ ★ **참고 자료는 ★ 원래 아무도 안 가리킨다.**  ★ 그것으로 죽었다고 세면 안 된다
```

| 무엇 | 안에 |
|---|---|
| `화면_승계문서.md` | ★ **v1 이 v2 에게 넘긴 문서** — 무엇이 있고 · 왜 그렇게 만들었고 · ★ **무엇을 지켜야 하는지** |
| `render/` | 화면 5개 실물 |
| `web/app.py` ＋ `templates/` | ★ 화면 10개 · 템플릿 12개 |

```
필수  ★ **지우지 않는다.**  ★ 마스터가 ★ UI 학습용으로 넣으신 것이다
필수  ★ 화면을 만들기 전에 ★ **`화면_승계문서.md` 4장(v2 로 옮길 때 지킬 것)을 읽는다**
금지  ★ 「아무도 안 가리킨다」를 ★ 지우는 근거로 쓰는 것 — ★ 참고 자료에는 안 맞는다
검산  ★ `S46-48` — ★ `ref/v1-ui/` 가 없으면 ★ 실패
```

## ★★ v1 이 「반드시 유지」라 한 다섯 — ★ 가이드가 대조했다 (08-24)

| # | v1 이 지키라 한 것 | 까닭 | ★ 지금 |
|:--:|---|---|:--:|
| 1 | ★ `/why` 판정 근거 화면 | ★ **이 도구의 존재 이유** | ★ 지킴 — 상세 안 24축 |
| 2 | ★ **색 4종 제한** | 늘리면 강조가 사라진다 | ★ 지킴 — 주황·초록·빨강·회색이 24회씩 기둥 (나머지는 그 색의 어두운 배경) |
| 3 | 숫자 고정폭 | 자릿수 비교가 안 되면 표가 무의미 | ★ 지킴 — `mono` |
| 4 | 빌드 도구 없음 | 유지보수 비용 | ★ 지킴 |
| 5 | 폴링 방식 진행률 | WebSocket 은 과잉 | ★ 지킴 |

```
★★ ★ **다섯 다 지키고 있다.**  ★ 그러나 ★ 가이드는 ★ 이 문서를 ★ **한 번도 안 읽고** 지켰다
   ★ ★ 우연히 맞은 것이다.  ★ 다음 가이드는 ★ **읽고** 지켜야 한다
```
