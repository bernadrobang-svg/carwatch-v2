...
| `spec.trim` 트림 | 59 | `modelTrim` · `modelMission` · `modelDoor` | 시그니처 그래비티 · A/T · 5인승 |
...
| `history.usage` 용도 | 22 | ★ `insuranceRecord.changeOfUse` | **0** |
...
| `spec.options` 옵션 | 43 | `mainOptions` (13종 · 이름) | LEATHER_SEATS · NAVIGATION · **SUNROOF** · ADAS … |
| `taste.sunroof` 선루프 | 12 | 〃 `SUNROOF` | true |
| `spec.trim` 트림 | 59 | `car.trim` · `car.modelName` | 시그니처 |
...
```
✘ `value.depreciation` 신차가 — 없다.  ★ 차종·트림 표에서 보충 (f-table 「사이트별 채우기」 ③)
✘ `value.market` 시세 — 우리 산출 (④)
✘ `history.not_join` 자차 미가입 · `history.owner` 소유자 변경 · `history.lien` 압류·저당
