...
# ★★★ 재판정은 ★ **하루 한 번 ＋ 네 시간마다** (마스터 확정 08-25 · 개정 735)

```
★★ 마스터 — 「★ **재판정이 하루 한 번(13:00)만 돕니다 ★ 가 기본이되
   ★ 적용 안 되면 ★ 네 시간 간격으로 다시 진행**」
```
...
| | 언제 | 무엇을 |
|---|---|---|
| ★ **기본** | ★ **날마다 13:00** | ★ 전량 재판정 (`carwatch-daily.timer`) |
| ★★ **채우기** | ★ **네 시간마다** | ★ **판정이 안 붙은 것만** |

```
★ ★ 채우기가 ★ 도는 조건 —
   ① ★ `result_score` 가 ★ **없는 매물**이 있다 (새로 들어온 것)
   ② ★ 또는 ★ `config` 가 ★ 바뀌었는데 ★ 재판정이 안 돌았다
   ③ ★ 또는 ★ 사전(`dict_enum`)이 ★ 확정됐는데 ★ 안 돌았다
★★ ★ **셋 다 아니면 ★ 안 돈다** — ★ 헛돌지 않는다

필수  ★ ★ **채우기는 ★ 안 붙은 것만** 한다 — ★ 전량을 다시 하지 않는다
      ★ ★ 전량은 ★ 13:00 에 한 번이면 된다
필수  ★ ★ 돌 때마다 ★ **감사에 남긴다** — ★ 「채우기 · N건 · 몇 초」
필수  ★ ★ **판본(`run` 번호)을 새로 만든다** — ★ 화면이 언제 것인지 보여야 한다
금지  ★ ★ **네 시간마다 전량을 다시 판정하는 것** — ★ 서버가 못 버틴다
```

## ★★ 왜 필요한가

```
★ 08-25 실측 — ★ 판본이 ★ `run 20260824T074027` 에서 ★ **멈춰 있었다**
★ ★ 그동안 ★ 사이트 열하나가 들어왔는데 ★ **판정이 안 붙었다**
★★ ★ 마스터께서 ★ **오늘 들어온 매물을 오늘 못 보신다**
   ★ ★ 좁혀 받기를 해도 ★ 판정이 하루를 기다리면 ★ 소용이 없다
```

## ★ 어디에 넣나

```
★ `carwatch-fill.timer` — ★ 새로 만든다 (★ `carwatch-daily.timer` 는 그대로)
★ ★ `OnCalendar=*-*-* 00,04,08,12,16,20:00:00`
★ ★ 13:00 과 ★ 겹치지 않는다 (★ 12:00 · 16:00 사이)
필수  ★ ★ **앞 실행이 안 끝났으면 ★ 건너뛴다** — ★ 겹쳐 돌지 않는다
검산  ★ `S46-65` — ★ 판본이 ★ 네 시간보다 오래됐으면 ★ 알린다
```
