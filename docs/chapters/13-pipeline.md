...
## STEP 49a [규격] — ★★ **다 못 받으면 ★ 이어 받는다** (개정 1102 · 09-02)

```
★★★ 규격 둘이 서로 걸렸다 —
   ① 「★ **욕심내지 않는다**」 — ★ 한 판에 몰아 받지 않는다 (쉼 · 묶음 상한)
   ② 「★ 사이트별 ★ **받은 비율 90%**」 — ★ S1 의 끝 조건
   ★ ★ 실측 09-02 — ★ KB 가 ★ **88.3% 에서 한 묶음(35 중 1)이 안 끝났다**
     ★ ★ ①을 지키면 ★ ②를 한 판에 못 넘긴다
★★ 갈랐다 — ★ **①이 먼저다.  ★ ②는 ★ 여러 판에 걸쳐 채운다**
```

| 무엇 | 어떻게 |
|---|---|
| 한 판에 다 못 받았다 | ★ **실패가 아니다** — ★ 「끝까지 받은 묶음 **N/M**」을 회차에 적고 ★ 다음 판에 **이어 받는다** |
| 끝 조건 | ★ **「그날 다시 세어 90%」** — ★ 분모를 박아 두지 않는다.  ★ 그 사이 팔린 차가 있다 |
| 금지 | ★ `--bursts` 를 늘려 ★ **한 판에 몰아 받는 것** |
| 금지 | ★ 「사이트가 막았다」를 ★ **두드린 증거 없이** 적는 것 |

```
★★★ 09-02 실측 — ★ KB 는 ★ **막지 않는다**
   robots      /public/search/list.empty · /public/car/detail.kbc ★ 금지 목록에 없다
               (막힌 것은 /public/review/car/detail.kbc — ★ 다른 경로다)
   목록        page 1·20·45 ★ 다 200 · carSeq 40
   쉼 없이 12번 ★ 11/12 가 40건 — ★ 안 막는다
   상세        /public/car/detail.kbc?carSeq=… ★ 200 · carSeq 109회 등장
★ ★ 마스터 인수인계 ㉰ — 「★ 개발측이 「막혔다」라 해도 ★ 그대로 옮기지 마라 — ★ 네가 두드려 봐라」
★ ★ ★ 08-24 에도 ★ **KB 「전건 봇 차단」이 ★ 마스터 실측에서 6/6 정상**이었다.  ★ 같은 자리다
검산  `S46-256`
```
