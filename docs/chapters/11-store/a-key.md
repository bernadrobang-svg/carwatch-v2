...
# ★★★★★ 08-29 — ★ **언제 「판매 완료」로 매기나** (마스터 물음)

```
★ 마스터 — 「★ 계속 삭제된 것들을 봐야 되는 거야?  ★ 목록이 없다고 삭제하지 않는다면
   ★ 어떤 기준으로 판매 완료를 처리할 거야?」
★ ★ 「확인한 뒤 죽인다」만 적고 ★ **언제까지 확인하나를 안 적었다** — ★ 그대로 두면 영영 안 죽는다
```

| 갈래 | 언제 | `status` | `sales_status` |
|---|---|---|---|
| 사이트가 말했다 | `sales_status` 가 「판매완료·계약중」 | `gone` | 사이트 원문 |
| 상세가 없어졌다 | 상세를 눌러 「없음」 | `gone` | `detail_gone` |
| 목록에서만 사라졌다 | 목록에 없는데 ★ 상세는 살아 있다 | ★ **`active` 그대로** | — |

```
필수  ★ **세 번 잇달아 목록에 없으면** ★ 상세를 눌러 위 표로 매긴다
      ★ ★ 한 번은 ★ 사이트가 잠깐 안 준 것일 수 있다 (08-29 · 74대)
필수  ★ 세는 법은 ★ `last_seen` 과 오늘 날짜의 **차**다 — ★ `miss_cnt` 를 새로 두지 않는다
필수  ★ **사흘 넘게 못 본 것만** 상세를 누른다 — ★ 날마다 전건을 안 누른다
필수  ★ 상세를 못 받으면(403·503·토큰) ★ **아무것도 안 한다.  ★ 다음 바퀴에**
필수  ★ ★ 다만 ★ **열흘 넘게 못 받으면** ★ `gone` ＋ `sales_status='unreachable'`
      ★ ★ **영영 안 죽는 매물을 만들지 않는다**
금지  ★ 한 번 사라졌다고 죽이는 것 (08-29 에 74대를 그렇게 죽였다)
검산  ★ `S46-169` — ★ `gone` 인데 ★ `sales_status` 가 빈 것이 없는가
      ★ ★ 빈 것은 ★ **왜 죽었는지 모르는 행**이다

★ ★ **「사흘」·「열흘」은 가이드 안이다** — ★ 재본 수가 아니다.
   ★ ★ 한 달을 돌려 ★ **며칠 만에 진짜 팔리는지** 잰 뒤 ★ 마스터께 올린다
```
