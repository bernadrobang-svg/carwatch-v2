...
| 무엇 | 어떻게 |
|---|---|
| ★ **내려받지 않는다** | ★ 그림 파일을 ★ **디스크에도 DB 에도 두지 않는다** |
| ★ 두는 곳 | ★ `core_listing.photo_list_json` — ★ **주소 목록**만 |
| ★ **상한 스무 장** | ★ 스물한 장째부터 ★ **버린다**.  ★ 앞에서부터 스물 |
| 화면 | ★ 그 주소를 ★ 그대로 `<img src>` 에 쓴다 |

```
★ 까닭 — ★ 그림을 받으면 ★ **매물 26,608건 × 40KB ≈ 1GB** 다.
  ★ ★ 장비 RAM 이 **1.8GB** 이고 ★ DB 는 이미 1.08GiB 인데 ★ **서버가 세 번 죽었다**
★ ★ 스무 장 상한이 없으면 ★ 한 매물이 ★ 주소만으로 **4,358자**까지 부푼다
  ★ ★ [실측 09-04 — 볼보셀렉트 최대 **4,358자** · KB **2,868자** · 엔카 508 · 리볼트 138]
```

★ **잃는 것도 적는다** (조용히 넘기지 않는다 · RULES.md 2)

```
★ 팔리면 ★ 사이트가 매물을 내려 ★ **주소가 죽는다** —
  ★ ★ 실측 09-04 — ★ `/sold` 화면의 사진 수가 ★ **0** 이다
★ ★ 그러니 ★ 팔린차 화면에는 ★ 「★ **그 사이트가 내려 사진이 없습니다**」를 낸다
  ★ ★ ★ **빈 자리로 두지 않는다** — ★ 까닭을 글로 낸다
검산  `S46-272`
```
