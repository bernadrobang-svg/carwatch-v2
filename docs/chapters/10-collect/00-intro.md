...
| **④** | ★ **`raw_response` 를 지운다 ＋ `VACUUM`** | — | ★★ **서버가 죽는다 — 세 번 죽었다** |
| **⑤** | ★ 적재 DB 파일을 지운다 | — | ★ 디스크가 찬다 |
...
## ★★★★★★ 적재 뒤 ★ **`raw_response` 를 지운다** (개정 1120 · 마스터 지시 09-03)
...
| ③ | 임시 DB 에서 ★ **파싱해 본 DB 에 넣는다** | `core_*` · `result_*` |
| ④ | ★★ **`raw_response` 를 지운다** | ★ 적재가 끝난 것만 |
| ⑤ | ★ 임시 DB 파일을 지운다 | ★ 판이 끝나면 |
