...
```
★ 사이트 소개 글 (인용) — 「★ **오직, 전기차를 위한 기술 인증 중고차**」 · 「★ 전기차 기술 진단(창구는 2장 `GET /cars/{hash}/inspection_records/`)을 통해
...
| `GET /cars/{hash}/carhistory/` | 200 | 보험이력 (창구 = 이 주소다) |
...
```
① ★ **대상에 넣을까** — ★ 전기차 인증중고차이고 ★ 마스터 조건과 겹친다
